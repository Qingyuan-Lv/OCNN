...
class STEQuantizer(torch.autograd.Function):
    @staticmethod
    def forward(ctx, w, discrete_values):
        w_flat = w.view(-1)
...
class QuantizedConv2d(nn.Conv2d):
    def __init__(self, *args, discrete_values, **kwargs):
        super().__init__(*args, **kwargs)
        self.discrete_values = discrete_values.to(device)

    def forward(self, x):
        quantized_weight = STEQuantizer.apply(self.weight, self.discrete_values)
...
class Gewu(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            QuantizedConv2d(1, 32, 3, padding=1, stride=2, bias=True, discrete_values=discrete_values),
            nn.ReLU(),
            QuantizedConv2d(32, 64, 3, padding=1, stride=2, bias=True, discrete_values=discrete_values),
            nn.ReLU(),
            QuantizedConv2d(64, 128, 3, padding=1, stride=2, bias=True, discrete_values=discrete_values),
            nn.ReLU(),
            nn.Flatten(),
            QuantizedLinear(128 * 4 * 4, 10, bias=True, discrete_values=discrete_values)
        )
