...
if __name__ == "__main__":
    model = torch.load("compressed_model_test_0507.pth", weights_only=False)
    model = model.to(device)
    print(f"model.conv1_1.sigma: {model.conv1_1.sigma}")
    print(f"model.classifier[2].loss_factor: {model.classifier[2].loss_factor}")
    print(f"model.max_sigma: {model.max_sigma}")
    print(f"model.max_loss_factor: {model.max_loss_factor}")
    ...
    print(
        f"加载后直接验证准确率: {current_acc:.2f}%, Current Sigma: {model.conv1_1.sigma:.4f}, Current Loss_factor: {model.classifier[2].loss_factor:.4f}")

    criterion = CrossEntropyLoss()
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=0.01)

    TOTAL_EPOCHS = 300
    SIGMA_WARMUP_EPOCHS = 30
    MAX_RETRY = 5
    ...
    best_acc = 0
    best_model_path = "best_compressed_model_0508.pth"
    ...
    while real_epoch < TOTAL_EPOCHS:
        ...
        if current_acc > best_acc and current_sigma == model.max_sigma and current_loss_factor == model.max_loss_factor:
        ...
        if retry_count == 0 and real_epoch > 0 and current_sigma > last_sigma and current_loss_factor > last_loss_factor:
            ...
            if acc_drop > ACC_DROP_THRESHOLD * 100:
                if retry_num < MAX_RETRY:
                    ...
                    retry_count += 1
                    retry_num += 1
                    epochs_plot.append(real_epoch)
                    accuracies_plot.append(current_acc)
                    is_rollback_plot.append(True)
                    sigmas_plot.append(current_sigma)
                    continue
                else:
